...
class SmsDemo
{
     ...
     */
    public static function getAcsClient($find_config=[]) {
        ...
        $accessKeyId = $find_config['sms_appid']; // AccessKeyId
        $accessKeySecret = $find_config['sms_appsecret']; // AccessKeySecret
     ...
     * Array
        (
            [sms] => sms
            [appkey] => LTAIJWkxF46NB5Af
            [secretkey] => 23mN6iBhTErPuRxFz7WUqKHjGk5a5x
            [type] => normal
            [name] => 淄川区党建办
            [code] => SMS_136871164
            [content] => 
     ...
     * LTAIo8P3EvbTdehj
     * 0lzdSelRQ8T5PrtGAtqgqXVVSGhzty
     * 出彩
     * SMS_142153366
     */
    public function sendSms($mobile,$code,$find_config=[]){
        $signName = $find_config['sms_signname'];
        $templateCode = $find_config['sms_code'];
        ...
        // 发起访问请求
        $acsResponse = self::getAcsClient($find_config)->getAcsResponse($request);
        
        return $acsResponse;
    }
    
    public function sendSmsContent($mobile,$name,$find_config=[]){
        $signName = $find_config['sms_signname'];
        $templateCode = $find_config['sms_code'];
