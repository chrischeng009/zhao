...
use app\common\model\AdminofflineModel;//本model放最后方便看清是否弄错

class Adminoffline extends Controller
{
    public function lists()
    {
        ...
        $query = AdminofflineModel::tbname()->where("1=1 $where")->order('id desc')->paginate($limit,false,$params);
        $page_show = $query->render();
        $sum = AdminofflineModel::summoney($_GET['admin_id']);
        $this->assign('sum',$sum);
    ...
    public function act_add()
    {
        ...
        $result = AdminofflineModel::do_add($params);
        echo $result;
    }
    public function act_del()
    {
        $this->isAuth();
        $result = AdminofflineModel::del("and id='".postt('id')."'");
        if($result){
            echo 'success';
        }else{
            echo '操作失败';
