...
class Shopcategory extends Controller
{
    public function lists()
    {
        ...
        $query = ShopcategoryModel::tbname()->where("1=1 $where")->order('id desc')->paginate($limit,false,$params);
    ...
    public function add()
    {
        ...
        $find = ShopcategoryModel::find("and id=".$_GET['id']);
    ...
    public function act_add()
    {
        ...
        $data['intime'] = time();
        $data['uptime'] = time();
        $result = ShopcategoryModel::add($data);
        if($result){
            echo 'success';
        }else{
            echo '操作失败';
    ...
    {
        $this->isAuth();
        $id = ShopcategoryModel::getChildId(postt('id'));
        $where = " and id in (".$id.")";
        $result = ShopcategoryModel::del($where);
        if($result){
            echo 'success';
        }else{
            echo '操作失败';
    ...
    {
        $this->isAuth();
        $find = ShopcategoryModel::find("and id=".gett('id'));
        $this->assign('find',$find);
        
        $find_parent = ShopcategoryModel::find("and id=".$find['pid']);
    ...
    public function act_edit()
    {
        ...
        unset($data['id']);
        $data['uptime'] = time();
        $result = ShopcategoryModel::edit("and id='".postt('id')."'",$data);
        if($result){
            echo 'success';
        }else{
            echo '操作失败';
